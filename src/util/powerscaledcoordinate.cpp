...
PowerScaledCoordinate::CreatePowerScaledCoordinate(double d1, double d2, double d3) //changed nh
{
//char buff[600];
...
if (ds >= 0.0) {  //Will always be 0 now. but will change when actual scale is implemented
double p = pow(k, -ds);
*this = PowerScaledCoordinate(static_cast<float>(-rhs._vec[0] * p + _vec[0]),
static_cast<float>(-rhs._vec[1] * p + _vec[1]),
static_cast<float>(-rhs._vec[2] * p + _vec[2]), _vec[3]);
